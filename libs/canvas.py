...
class Canvas(QWidget):
    ...
    def mouseMoveEvent(self, ev):
        ...
        if self.panning and ev.buttons() in (Qt.MiddleButton, Qt.LeftButton):
            ...
            # if self.selected_shape and self.prev_point:
            #     self.override_cursor(CURSOR_MOVE)
            #     self.bounded_move_shape(self.selected_shape, pos)
            #     self.shapeMoved.emit()
            #     self.repaint()
    ...
    def wheelEvent(self, ev):
        ...
        if v_delta:
            self.zoomRequest.emit(v_delta, pos.x(), pos.y(), ev.pos().x(), ev.pos().y())
        else:
    ...
    def discard_moving(self):
        self.moving_box = False
        self.moving_vertex = False
        if self.shape_before_edit and self.selected_shape:
    ...
    def move_one_pixel(self, direction):
        pixels = 1.0
        if direction == 'Left' and not self.move_out_of_bound(QPointF(-1.0 * pixels, 0)):
        ...
        elif direction == 'Right' and not self.move_out_of_bound(QPointF(1.0 * pixels, 0)):
            # print("move Right one pixel")
            self.selected_shape.points[0] += QPointF(1.0, 0)
            self.selected_shape.points[1] += QPointF(1.0, 0)
            self.selected_shape.points[2] += QPointF(1.0, 0)
            self.selected_shape.points[3] += QPointF(1.0, 0)
        elif direction == 'Up' and not self.move_out_of_bound(QPointF(0, -1.0 * pixels)):
        ...
        elif direction == 'Down' and not self.move_out_of_bound(QPointF(0, 1.0 * pixels)):
